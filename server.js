...
// Pending settings password verifications.
//
// Key:
// DeviceID:nonce
//
...
const pendingSettingsVerifications =
  new Map();
...
app.post(
  ...
  async (req, res) => {
    ...
    // Generate a nonce so that the response
    // from the ESP32 can be matched to this
    // exact request.
  ...
  (req, res) => {
    ...
    if (
      ...
      return res
        ...
        .json({
          error:
            "Invalid Steam LED range"
        });
    }

    // Password is validated by the ESP32,
    // not by the backend.
